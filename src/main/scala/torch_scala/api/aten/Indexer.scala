...
import torch_scala.api.exception.InvalidIndexerException
          ...
          while (indexers.length - newRank + j > i) {
            ...
            indexers(indexers.length - newRank + j) match {
              ...
              case NewAxis =>
                ...
                newAxesCounter += 1
              case Index(index) =>
                beginOffsets(j) = if (index < 0) index + oldDimSize.toInt else index
                endOffsets(j) = beginOffsets(j) + 1
                strides(j) = 1
                dimensions(j) = 1
                oldDimensions(j) = oldDimSize.toInt
              case s@Slice(begin, end, step, inclusive) =>
                beginOffsets(j) = if (begin < 0) begin + oldDimSize.toInt else begin
                val effectiveEnd = if (end < 0) end + oldDimSize else end
                endOffsets(j) = if (inclusive) effectiveEnd.toInt + 1 else effectiveEnd.toInt
                strides(j) = step
